    ...
    public void playerJoinEvent(PlayerJoinEvent joinEvent) {
        ...
        for (Base base : Game.getBases()) {
            if (base.hasPlayer(player)) {
                ...
                Message.broadcast( ChatColor.YELLOW + player.getDisplayName() + Main.getLangFile().getString("message.isback1") + base.getNameString() + ChatColor.YELLOW + Main.getLangFile().getString("message.isback2"));
        ...
        if (Game.isRunning()) {
            ...
            Message.broadcast( ChatColor.GRAY + player.getDisplayName() + Main.getLangFile().getString("message.spectator"));
            joinEvent.setJoinMessage(null);
        } else {
            if (Main.getConfigFile().getBoolean("autoStart")) {
                ...
                Message.broadcast( ChatColor.YELLOW + player.getDisplayName() + Main.getLangFile().getString("message.join") + " [" + playernum + "/" + Main.getConfigFile().getInt("maxPlayer") + "]");
                joinEvent.setJoinMessage(null);
            } else {
                Message.broadcast( ChatColor.YELLOW + player.getDisplayName() + Main.getLangFile().getString("message.join"));
                joinEvent.setJoinMessage(null);
            }
